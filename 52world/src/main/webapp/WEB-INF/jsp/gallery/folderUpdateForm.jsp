...
<div class="middle-container">
	...
	<c:if test="${empty Homepee.homepeeId}">
		<c:redirect url="${pageContext.request.contextPath }/galleryUpdateFolder.do"></c:redirect>
	</c:if> 


		<h1>폴더수정 페이지입니다.</h1>
			<form id="editAlbum" action="${pageContext.request.contextPath }/galleryUpdateFolder.do" method="post" >
				...
				<div class = "title">
						<h3>
							<label for="folderName">변경할 이름</label>
				...
				<div class="btn">
					<input id="update" type="button" value="변경" onclick="editCheck()">
	...
	<script type="text/javascript">
		...
		function editCheck(){
			if(editAlbum.fname.value==""){
				alert("폴더명을 입력해주세요.");
				editAlbum.fname.focus();
	   			return false;
			}else{
	   			
	   			
				let cf = confirm('폴더를 수정하시겠습니까?')
				if(cf){
					alert("폴더가 수정되었습니다.");
					editAlbum.submit();
					
				} else {
					alert("폴더 수정을 취소하였습니다")
				}
	   		}
		}
